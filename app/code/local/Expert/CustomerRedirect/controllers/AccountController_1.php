...
class Expert_CustomerRedirect_AccountController extends Mage_Customer_AccountController
{
    ...
    protected function _loginPostRedirect()
    {
		...
		$cusromerRedirectStatus = Mage::getStoreConfig('customer/customerloginredirect/active1');
        ...
        if ($this->_getSession()->isLoggedIn()) {
			...
			//$this->_redirectUrl('/onlinesales/index.php/default/');
            return;
        }
	...
	public function loginPostCustomAction()
    {
		$loginUrl = $this->getRequest()->getPost('afterLoginUrl');
        if ($this->_getSession()->isLoggedIn()) {
            $this->_redirect('*/*/');
            return;
        }
        ...
        $this->_loginPostCustomRedirect($loginUrl);
    }
	
	
	 protected function _loginPostCustomRedirect($loginUrl)
    {
		$session = $this->_getSession();  
		$preUrl = $session->getBeforeAuthUrl(true);
        ...
        //echo $outputMessage = Mage::getSingleton('core/session')->getShowWizard();
        $this->_redirectUrl($preUrl);          
    }
